...
function asset(path) {
  const base = (typeof window !== "undefined" && window.BMCHESS_BASE)
    ? String(window.BMCHESS_BASE).replace(/\/$/, "")
    : "";
...
const WAR_KNIGHT_DIR = asset("immagini-stile-war/cavallonero");
const WAR_PAWN_DIR = asset("immagini-stile-war/pedonennero");
const WAR_BISHOP_DIR = asset("immagini-stile-war/alfierennero");
const WAR_ROOK_DIR = asset("immagini-stile-war/torrenera");
const WAR_QUEEN_DIR = asset("immagini-stile-war/reginanera");
const WAR_KING_DIR = asset("immagini-stile-war/renero");
const WAR_CASTLE_DIR = asset("immagini-stile-war/arrocconero");
...
const WAR_W_KNIGHT_DIR = asset("immagini-stile-war/cavallobianco");
const WAR_W_PAWN_DIR = asset("immagini-stile-war/pedonebianco");
const WAR_W_BISHOP_DIR = asset("immagini-stile-war/alfierobianco");
const WAR_W_ROOK_DIR = asset("immagini-stile-war/torrebianca");
const WAR_W_QUEEN_DIR = asset("immagini-stile-war/reginabianca");
const WAR_W_KING_DIR = asset("immagini-stile-war/rebianco");
...
const FUMETTO_W_KNIGHT_DIR = asset("immagini-stile-fumetto/cavallobianco");
const FUMETTO_W_PAWN_DIR = asset("immagini-stile-fumetto/pedonebianco");
const FUMETTO_W_BISHOP_DIR = asset("immagini-stile-fumetto/alfierobianco");
const FUMETTO_W_ROOK_DIR = asset("immagini-stile-fumetto/torrebianca");
const FUMETTO_W_QUEEN_DIR = asset("immagini-stile-fumetto/reginabianca");
const FUMETTO_W_KING_DIR = asset("immagini-stile-fumetto/rebianco");
const FUMETTO_W_CASTLE_DIR = asset("immagini-stile-fumetto/arroccobianco");
const FUMETTO_KNIGHT_DIR = asset("immagini-stile-fumetto/cavallonero");
const FUMETTO_PAWN_DIR = asset("immagini-stile-fumetto/pedonennero");
const FUMETTO_BISHOP_DIR = asset("immagini-stile-fumetto/alfierennero");
const FUMETTO_ROOK_DIR = asset("immagini-stile-fumetto/torrenera");
const FUMETTO_QUEEN_DIR = asset("immagini-stile-fumetto/reginanera");
const FUMETTO_KING_DIR = asset("immagini-stile-fumetto/renero");
const FUMETTO_CASTLE_DIR = asset("immagini-stile-fumetto/arrocconero");
...
function evalSwordSvg() {
  return `<img class="eval-sword" src="${asset("img/eval-sword.svg")}" alt="" aria-hidden="true">`;
}
...
function oppKingPieceSrc() {
  ...
  return asset(`pieces/${me === "w" ? "b" : "w"}K.svg`);
}
...
function pieceIcon(move, color) {
  if (!move) return "";
  if (move.san.startsWith("O-O")) return asset(`pieces/${color}K.svg`);
  const type = (move.piece || "p").toUpperCase();
  return asset(`pieces/${color}${type}.svg`);
}
...
function iconArtHtml(move, color) {
  const src = pieceIcon(move, color) || asset(`pieces/${color || "w"}N.svg`);
...
function adoptHintPool(pool, { freezeStand = false } = {}) {
  ...
  state.hintPool = pool || [];
  if (!freezeStand) state.lastPickedUci = "";
}
...
function swordPieceHtml(halves, i) {
  const fill = heartFill(halves, i);
  const kind = heartKind(halves, i);
  return `<span class="king-sword-piece is-${kind}" aria-hidden="true"><img class="sword-bg" src="${asset("img/eval-sword.svg")}" alt=""><span class="sword-fg" style="width:${(fill * 100).toFixed(2)}%"><img src="${asset("img/eval-sword.svg")}" alt=""></span></span>`;
}
...
function evalBarIconMarkup(kind) {
  ...
  if (kind === "sword" || kind === "skull") {
    const src = asset(kind === "skull" ? "img/eval-skull.png" : "img/eval-sword.svg");
...
function previewHintsSorted(pool = state.hintPool) {
  return (pool || [])
    .filter((hint) => hint?.uci && !hint.synthetic)
    .slice(0, hintPoolSize())
...
function previewEvalHtml(hint) {
  ...
  const text = isAbsLikeEval() ? formatHintEval(hint) : formatHintDelta(hint);
...
function adminBarBlackBottom() {
  if (isLocalVsHuman()) return state.board?.orientation === "black";
...
function paintAdminEvalBar() {
  ...
  const whiteCp = adminEngineWhiteCp();
  const whitePct = adminEngineWhitePct(whiteCp);
  const blackBottom = adminBarBlackBottom();
  const bottomPct = blackBottom ? 100 - whitePct : whitePct;
  els.adminEval?.classList.toggle("is-black-bottom", blackBottom);
  if (els.adminEvalTop) els.adminEvalTop.textContent = t(blackBottom ? "admin.eval.white" : "admin.eval.black");
  if (els.adminEvalBottom) els.adminEvalBottom.textContent = t(blackBottom ? "admin.eval.black" : "admin.eval.white");
  if (fill) fill.style.height = `${bottomPct.toFixed(2)}%`;
  if (score) {
    score.textContent = formatAdminEngineScore(whiteCp);
    score.classList.toggle("is-white", whiteCp > 8);
    score.classList.toggle("is-black", whiteCp < -8);
  }
  if (els.adminEval) {
    els.adminEval.setAttribute("aria-valuenow", String(Math.round(bottomPct)));
    els.adminEval.setAttribute("title", formatAdminEngineScore(whiteCp));
  }
}
...
const els = {
  ...
  adminEvalTop: document.getElementById("admin-eval-top"),
  adminEvalBottom: document.getElementById("admin-eval-bottom"),
...
function syncCoach() {
  ...
  if (els.kingPiece) els.kingPiece.src = asset(color === "b" ? "pieces/bK.svg" : "pieces/wK.svg");
  if (els.kingTitle) {
...
function graveHtml(types, victimColor, advantage) {
  const icons = types.map((type) => (
    `<img class="grave-piece" src="${asset(`pieces/${victimColor}${String(type).toUpperCase()}.svg`)}" alt="">`
  )).join("");
    ...
    .map(
      (p) =>
        `<button type="button" data-piece="${p}" title="${pieceName(p)}">
           <img src="${asset(`pieces/${color}${p.toUpperCase()}.svg`)}" alt="${pieceName(p)}">
         </button>`
    )
    .join("");
